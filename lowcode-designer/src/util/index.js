...
export function showFieldTypeDesc(item) {
  // debugger
  let str = ''
  if (item.sysAdd) {
    return '系统字段'
  ...
  if (item.type === 'input') {
    switch (item.options.validateType) {
      case 'maxlength':
        str = '单行'
        break
      case 'name':
    ...
    str = item.options.multiple ? '成员-多选' : '成员-单选'
  } else if (item.type === 'relatedChoice') {
    str = item.options.choiceType === 'multiple' ? '关联选项-多选' : '关联选项-单选'
  } else {
    str = item.options.typeDesc
  }
